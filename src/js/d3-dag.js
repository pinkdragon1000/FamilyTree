(function (global, factory) {
  ...
  function greedy() {
    ...
    function coordGreedy(layers, separation) {
      ...
      const span =
        ...
        min;
      layers.forEach((layer) =>
        layer.forEach((n) => (n.x = (n.x - min) / span))
      );
  ...
  var wrapper = function (qmat, cvec, amat, bvec, meq = 0, factorized = false) {
    ...
    const Amat = [null].concat(
      amat.length === 0
        ? new Array(qmat.length).fill([null])
  ...
  function adjacent() {
    ...
    function columnIndexAssignmentAdjacent(layers) {
      ...
      function optimizeColumnIndices(desiredColumnIndices) {
        if (
          !desiredColumnIndices.every((columnIndex) => isFinite(columnIndex))
        ) {
  ...
  function count() {
    this.eachAfter((node) => {
      if (node.children.length) {
        node._leaves = Object.assign(
          {},
          ...node.children.map((c) => c._leaves)
        );
  ...
  function verify(root) {
    ...
    function visit(node) {
      if (seen[node.id]) {
        ...
        past[node.id] = true;
        let result = node.children.reduce(
          (chain, c) => chain || visit(c),
          false
        );
        delete past[node.id];
        seen[node.id] = true;
    ...
    function dagStratify(data) {
      ...
      const nodes = data.map((datum, i) => {
        ...
        try {
          ...
          throw Error(
            `node ids must have toString but got ${nid} from ${datum}`
          );
        }
      });
  ...
  Tableau.prototype.getSolution = function () {
    var evaluation =
      this.model.isMinimization === true ? this.evaluation : -this.evaluation;

    if (this.model.getNumberOfIntegerVariables() > 0) {
      return new MilpSolution_1(
        this,
        evaluation,
        this.feasible,
        this.bounded,
  ...
  Tableau_1.prototype.addCutConstraints = function (cutConstraints) {
    ...
    for (var i = 0; i < nCutConstraints; i += 1) {
      ...
      this.variablesPerIndex[slackVarIndex] = new SlackVariable$1(
        "s" + slackVarIndex,
        slackVarIndex
      );
      this.nVars += 1;
  ...
  Tableau_1.prototype.updateConstraintCoefficient = function (
    constraint,
    variable,
    difference
  ) {
  ...
  Model.prototype.equal = function (rhs) {
    var constraintUpper = new Constraint$1(
      rhs,
      true,
      this.tableau.getNewElementIndex(),
      this
    ...
    var constraintLower = new Constraint$1(
      rhs,
      false,
      this.tableau.getNewElementIndex(),
      this
  ...
  function to_JSON(input) {
    var rxo = {
        ...
        parse_lhs:
          /(\-|\+){0,1}\s{0,1}\d{0,}\.{0,}\d{0,}\s{0,}[A-Za-z]\S{0,}/gi,
      ...
      for (var y in model.constraints[x]) {
        for (var z in model.variables) {
          ...
          if (model.variables[z][x]) {
            output +=
              " " + model.variables[z][x] + " " + z.replace(rxClean, "_");
          }
        }
  ...
  var FastPriorityQueue_1 = createCommonjsModule(function (module) {
    ...
    FastPriorityQueue.prototype.remove = function (myval, comparator) {
      ...
      for (var i = 0; i < this.size; i++) {
        if (
          comparator(this.array[i], myval) ||
          comparator(myval, this.array[i])
        ) {
          continue;
        }
